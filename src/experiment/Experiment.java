...
/**
 * The main class, used for all experiment
 * Static variable:
 * <ul>
 ...
 * <li><b>REPEAT	  </b> - how many time experiment should be repeat (with this same size)</li>
...
public class Experiment {
	...
	public static String OUTPUT_FILE = "co.txt";
	...
	for(int i = MIN_SIZE; i <= MAX_SIZE; i+=100)
	{
		...
		{
			//*******************************************************
			algorithm = new QuickSortFacade(PRINT_RESULT, i, OPTION); // here change variant
			//*******************************************************	
...
if(j == REPEAT -1)
{
			buffer=Integer.toString(i) + " " + Double.toString(avg(avg)) + "\n";
			System.out.println("size: " + i + " time: " + avg(avg));
//			WriteFile.writeString(OUTPUT_FILE, buffer);
			wf.writeString(buffer);
}
		}
//		}
